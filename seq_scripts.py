...
def seq_train(loader, model, optimizer, device, epoch_idx, recoder):
    ...
    for batch_idx, data in enumerate(loader):
        ...
        ret_dict = model(vid, vid_lgt, label=label, label_lgt=label_lgt, train_flag=True)
        loss, ctc_a, ctc_m, ctc_c = model.criterion_calculation(ret_dict, label, label_lgt)
        if np.isinf(loss.item()) or np.isnan(loss.item()):
            print(data[-1])
            continue
        optimizer.zero_grad()
        loss.backward()
        ctc_auxi = ctc_auxi + ctc_a
        ctc_main = ctc_main + ctc_m
        ctc_cross = ctc_cross + ctc_c
        optimizer.step()
...
def seq_eval(cfg, loader, model, device, mode, epoch, work_dir, recoder, evaluate_tool="python"):
    ...
    for batch_idx, data in enumerate(tqdm(loader)):
        ...
        #
        # total_text += ret_dict['word1']  #
        # total_text += ret_dict['word2']  #
    ...
    try:
        ...
        # write2file(work_dir + "output-hypothesis-{}-text.ctm".format(mode), total_info, total_text)  #
        conv_ret = evaluate(
        ...
        # text_ret = evaluate(
        #     prefix=work_dir, mode=mode, output_file="output-hypothesis-{}-text.ctm".format(mode),
        #     evaluate_dir=cfg.dataset_info['evaluation_dir'],
        #     evaluate_prefix=cfg.dataset_info['evaluation_prefix'],
        #     output_dir="epoch_{}_result/".format(epoch),
        #     python_evaluate=python_eval,
